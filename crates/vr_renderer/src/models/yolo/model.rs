...
use crate::models::yolo::{check_font, gen_time_string, non_max_suppression, Batch, Bbox, Embedding, OrtBackend, OrtConfig, OrtEP, Point2, YOLOResult, YOLOTask, SKELETON, Args};
...
impl YOLO {
    ...
    pub fn simd_preprocess(&mut self, xs: &Vec<DynamicImage>) -> Result<Array<f32, IxDyn>> {
    ...
    pub fn run(&mut self, xs: &Vec<DynamicImage>) -> Result<Vec<YOLOResult>> {
        ...
        let xs_ = self.simd_preprocess(xs)?;
        if self.profile {
